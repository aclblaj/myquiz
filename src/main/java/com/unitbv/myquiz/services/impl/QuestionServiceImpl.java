...
//            e.printStackTrace();
    ...
    public List<Question> getQuestionsForAuthorName(String authorName) {
        // get all author quizzes
        List<QuizAuthor> quizAuthors = quizAuthorRepository.findByAuthor_NameContainsIgnoreCase(authorName);
...
//        findAllQuestions();
        authors.forEach(author -> {
            questions.set(getQuestionsForAuthorId(author.getId(), course));
            ...
            logger.atInfo()
                  ...
                  .addArgument(questions.get().size())
    ...
    private List<QuizError> detectAuthorErrors(AtomicReference<List<Question>> questions) {
